...
│  │ Desktop  │  │ curio    │  │ agent_ex │  │ FileStrg │ │
...
```
Curio（按 user_id 统一，跨 Agent）

数据分层：
┌─ 热存储 ──────────────────────────────┐
│  entries 表（MySQL / SQLite）          │
│    ← 条目元数据 + 纯文本内容            │
│  timelines 表                           │
│    ← 用户创建的子时间线                  │
│  references 表                          │
...
CREATE TABLE curio_entries (
    ...
    type        VARCHAR(16) NOT NULL,            -- 'note' / 'photo' / 'recording' / 'link' / 'canvas'
    title       TEXT,                            -- VLM 提取 / 用户自填
    content     TEXT,                            -- 纯文本内容（OCR/ASR/自填）
    summary     TEXT,                            -- VLM 摘要
    tags        JSON,                            -- ['化学', '氧化还原']
    status      VARCHAR(16) DEFAULT 'active',    -- 'active' / 'archived' / 'processing'
    source      VARCHAR(32),                     -- 'manual' / 'photo' / 'recording' / 'link' / 'canvas'
    source_url  TEXT,                            -- 原始 URL（link 类型时）
    attachment  JSON,                            -- { bucket, key, mime, size }
    bbox        JSON,                            -- content_bbox { x_min, y_min, x_max, y_max }
    metadata    JSON,                            -- 扩展字段
    vector_id   VARCHAR(64),                     -- 对应 idx-curio 中的向量 ID
    created_at  DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at  DATETIME DEFAULT CURRENT_TIMESTAMP,
    archived_at DATETIME,

    INDEX idx_curio_user_id (user_id),
    INDEX idx_curio_created_at (created_at),
    INDEX idx_curio_type (user_id, type),
    FULLTEXT idx_curio_content (content)
);
```
...
```sql
CREATE TABLE curio_timelines (
    id          VARCHAR(26) PRIMARY KEY,         -- ULID
    user_id     INTEGER NOT NULL,
    name        VARCHAR(128),                    -- 用户命名
    description TEXT,
    type        VARCHAR(16) DEFAULT 'custom',    -- 'auto'(Agent 创建) / 'custom'(用户创建)
    created_at  DATETIME DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_timeline_user (user_id)
);

-- 关联表：条目 ←→ 时间线
CREATE TABLE curio_timeline_entries (
    timeline_id   VARCHAR(26) NOT NULL,
    entry_id      VARCHAR(26) NOT NULL,
    sort_order    INTEGER DEFAULT 0,
    added_at      DATETIME DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (timeline_id, entry_id),
    INDEX idx_tle_entry (entry_id)
);
```
...
```sql
CREATE TABLE curio_references (
    id            VARCHAR(26) PRIMARY KEY,
    source_id     VARCHAR(26) NOT NULL,          -- 引用方条目
    target_id     VARCHAR(26) NOT NULL,          -- 被引用方条目
    created_at    DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE INDEX idx_ref_pair (source_id, target_id),
...
|:--------|:-------------------------|
| 原始照片 | `curio/user_{uid}/attachments/{entry_id}_original.jpg` |
| 智能二值化图 | `curio/user_{uid}/attachments/{entry_id}_clean.webp` |
| VLM→HTML | `curio/user_{uid}/attachments/{entry_id}_render.html` |
| 录音 | `curio/user_{uid}/attachments/{entry_id}_audio.mp3` |
| 画布 strokes | `curio/user_{uid}/pages/{page_id}.json` |
| 画布缩略图 | `curio/user_{uid}/pages/{page_id}_thumb.webp` |

### 4.6 向量索引

```
索引名称：idx-curio-{uid}
文档内容：entry.content + entry.title + tags（拼接成纯文本）
元数据：  { entry_id, type, created_at }
路由模型：Qwen 3.6 Flash（与公共知识搜索共用，但限搜 idx-curio-{uid}）
```
...
输入：新条目的文本内容（ASR/OCR/自填）
   ↓
① 向量搜索 idx-curio-{uid} → Top 10（语义匹配）
...
CREATE TABLE user_profiles (
    ...
    observed_by_curio JSON DEFAULT '{}',
...
| `idx-curio-{uid}` | Curio 条目 | 用户私有，跨 Agent |
...

{
  ...
  "message": "帮我解这道题",
  "channel": "mobile",
  "attachments": ["curio://entry_xxx.jpg"]
}
...
```http
POST /api/curio/entries
{
  "type": "photo",
  "attachment": { "mime": "image/jpeg", "data": "base64..." }
}
→ { "id": "ulid", "status": "processing", "created_at": "..." }

GET /api/curio/entries?user_id=1&type=photo&limit=20&before=timestamp
→ { "entries": [...], "has_more": true }

POST /api/curio/entries/{id}/archive
→ { "status": "archived" }
```
...
```http
PUT /api/curio/pages/{page_id}
Content-Type: application/json

{ "strokes": [...], "metadata": {...} }

GET /api/curio/pages/{page_id}
...
| 周期 | 内容 |
|:----|:-----|
| Week 1-2 | Agent 聊天现有功能稳定 + Curio 条目 CRUD + 时间线 |
| Week 3-4 | Curio 画布 v1（缩放 2x、<5000 笔、JSON 存储、缩略图冷启动、evenodd 导出） |
| Week 5-6 | Curio 拍照入库管线（印刷体 → VLM→HTML） |
