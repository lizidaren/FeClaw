...
class WeChatBot:
    ...
    async def reply(self, msg: IncomingMessage, text: str) -> None:
        ...
        """
        self._context_tokens[msg.user_id] = msg.context_token
        await self._send_content(msg.user_id, msg.context_token, text)
        try:
            await self.stop_typing(msg.user_id)
        except Exception:
            logger.debug(f"stop_typing failed for {msg.user_id}")
    ...
    async def reply_media(self, msg: IncomingMessage, content: SendContent) -> None:
        ...
        """
        self._context_tokens[msg.user_id] = msg.context_token
        await self._send_content(msg.user_id, msg.context_token, content)
        try:
            await self.stop_typing(msg.user_id)
        except Exception:
            logger.debug(f"stop_typing failed for {msg.user_id}")
    ...
    async def start(self) -> None:
        ...
        while not self._stopped:
            try:
                ...
                # Call heartbeat callback to signal the loop is alive
                if self._on_heartbeat:
                    try:
                        self._on_heartbeat()
                    except Exception:
                        logger.debug("Heartbeat callback failed")
