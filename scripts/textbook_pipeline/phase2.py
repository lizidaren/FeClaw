...
async def call_ds(model, prompt, user_content, thinking=True, timeout=310, raw_output=False):
    """Call DeepSeek API.
    ...
    async def process_one(idx, chunk):
        async with sem:
            ...
            if len(text) < 100:
                ...
                return [chunk]
            numbered, _ = add_numbers(text)
            l3_res, el = await call_ds('deepseek-v4-pro', L3_PROMPT, numbered, timeout=180)
...
async def phase4(chunks):
    ...
    if large:
        print(f'  [SKIP] Pro summary: {len(large)} large chunks (using raw_text directly)')
...
async def phase5(chunks):
    ...
    for subject, c_list in sorted(by_subject.items()):
        idx = f'idx-public-{subject}-textbook'
